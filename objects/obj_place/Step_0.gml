...
if global.edit_enabled == false // Prevents tiles from being manipulated when edit mode is enabled
{
	if obj_grid.mouse_in_grid == true // Ensures tiles outside grid are not manipulated.
	{
		if mouse_check_button_pressed(global.controls[0]) // Checks for mouse action button click
		{
			// SETS TILE MANIPULATIONS BACK TO DEFAULT
			
			if ds_grid_get(collide_grid_index, obj_grid.x_pos, obj_grid.y_pos) == 0
			{
				obj_buttons.stuck[11] = false; // Unstick rotate
				obj_buttons.stuck[12] = false; // Unstick flip horizontal
				obj_buttons.stuck[13] = false; // Unstick flip vertical
			
				global.rotate_enabled = false; // Disable rotation
				global.flip_vertical_enabled = false; // Disable flip vertical
				global.flip_horizontal_enabled = false; // Disable flip horizontal
			}
			
			ds_grid_set(collide_grid_index, obj_grid.x_pos, obj_grid.y_pos, obj_tiles.last_selection); // Sets tile index to last selection
			
			spt_sound(0, 4); // Play tile placement sound
		...
		if mouse_check_button_pressed(global.controls[1]) // Checks for delete mouse click
		{
			...
			spt_sound(0, 3); // Play tile delete sound
		}
	}
}
